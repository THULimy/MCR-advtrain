...
def load_architectures(name, dim):
    ...
    if _name == "resnet18":
        ...
        net = LeNet(dim)
    elif _name == "lenet_with_clf":
        from architectures.lenet import LeNet_with_clf
        net = LeNet_with_clf(dim)
    elif _name == "smallcnn":
...
def get_dataset_with_specific_class(trainset, select_labels):
    '''
    train_set: {some dataset with .targets and .data}
    seletct_labels: {list}
    '''
    targets = np.array(trainset.targets)
    select_index = np.isin(targets, select_labels)
    trainset.data = trainset.data[select_index]
    trainset.targets = targets[select_index].tolist()
    return trainset